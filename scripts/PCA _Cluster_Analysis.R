...
### PCA on glasshouse variables 
# LDMC instead of succulence
# add leaf density (LMA/thickness) see Bartlett et al. 2012
...
LDMCandDensity <- LMA %>%
  ...
  mutate(LDMC = (Dry_Weight_g/Fresh_Weight_g),
         Density = (LMA_gm2/Thickness_mm_Avg)) %>%
  group_by(Species_Code) %>%
  summarise(mean_LDMC = mean(LDMC, na.rm = TRUE),
            mean_Density = mean(Density, na.rm = TRUE))

# join to master spreadsheet
PCA_analysis <- left_join(other.variables, LDMCandDensity, by = "Species_Code")
...
eig.val <- get_eigenvalue(res.pca)
eig.val
# first 3 PCs explain 87% of the data
...
res.desc <- dimdesc(res.pca, axes = c(1,2), proba = 0.05)
# Description of dimension 1
res.desc$Dim.1 # LDMC and osmpot and density
# Description of dimension 2
res.desc$Dim.2 # thickness and LMA
...
# load the fv/fm data and sort out

fvfm <- read.csv("GH_data/WPW_GH_FVFM.csv")

fvfm <- fvfm %>%
  select(Species_Code, Treatment, Sample_ID, Fv.Fm) %>%
  filter(Treatment == "C" | Treatment == "D") %>%
  group_by(Species_Code, Treatment) %>%
  summarise(mean_fvfm = mean(Fv.Fm, na.rm = TRUE))

fvfm_long <- spread(fvfm, Treatment, mean_fvfm) %>%
  rename(Control=C, Drought=D) %>%
  mutate(delta_fvfm = Control - Drought)

# there is lots of NAs for the delta as there are lots of drought values missing
# let's just use control values (only Mypa is missing)

fvfm_long <- fvfm_long %>%
  select(Species_Code, Control) %>%
  filter(Species_Code != "Mypa")

other.variables <- read.csv("PCA_Cluster_data/PCA_data.csv")

other.variables <- filter(other.variables, Species_Code != "Crma", Species_Code != "Atfi",
                          Species_Code != "Brru", Species_Code != "Mypa") # filter out species with missing leaf area and fvfm
...
# join fvfm data
PCA_analysis <- left_join(PCA_analysis, fvfm_long, by = "Species_Code")

PCA_analysis <- rename(PCA_analysis, mean_C_fvfm = Control)
...
# might need to transform the thickness, fvfm and area variables

PCA_analysis_pairs <- PCA_analysis_pairs %>%
  mutate(log_Thickness_mm_Avg = log(Thickness_mm_Avg),
         log_leaf_area_cm2 = log(mean_leaf_area_cm2),
         log_mean_C_fvfm = log(mean_C_fvfm)) %>%
  select(-Thickness_mm_Avg, -mean_leaf_area_cm2, -mean_C_fvfm)

pairs(PCA_analysis_pairs) # fvfm still looks bad
...
PCA_analysis <- PCA_analysis %>%
  mutate(log_Thickness_mm_Avg = log(Thickness_mm_Avg),
         log_leaf_area_cm2 = log(mean_leaf_area_cm2),
         log_mean_C_fvfm = log(mean_C_fvfm)) %>%
  select(-Thickness_mm_Avg, -mean_leaf_area_cm2, -mean_C_fvfm, -Species)
...
# save the output
pdf("PCA_Cluster_output/glasshouse_no_climate_LDMC_TLP_FVFM.pdf") # Create a new pdf device
print(var.plot)
print(ind.plot)
print(cluster)
dev.off() # Close the pdf device

# fvfm really doesn't add anything
